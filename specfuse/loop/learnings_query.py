...
from specfuse.loop.closing_requirements import LEARNINGS_PATH
...
def default_learnings_path() -> Path:
    """The project's LEARNINGS.md, resolved against the current directory.

    Resolved per call, not at import: this module ships inside the installed
    package, so anything derived from ``__file__`` would point into
    site-packages rather than at the project being worked on. Every other
    driver module locates project state the same way — relative to the
    directory the command was run from — and ``closing_requirements`` already
    owns the canonical relative path.
    """
    return Path(LEARNINGS_PATH)
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to a LEARNINGS.md-formatted file "
             "(default: .specfuse/LEARNINGS.md under the current directory).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    path = args.file if args.file is not None else default_learnings_path()

    try:
        text = path.read_text(encoding="utf-8")
    except OSError as exc:
        print(f"learnings_query: cannot read {path}: {exc}", file=sys.stderr)
        return 1

    entries = parse_entries(text)
    if not entries:
        print(f"learnings_query: no entries found in {path}", file=sys.stderr)
        return 1
