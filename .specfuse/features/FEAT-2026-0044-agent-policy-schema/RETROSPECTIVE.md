...
- **Operator reason (verbatim):** "accepting since D2 and D3 are already verified and the following feature will exercise it"
- **Contract-change list:** acknowledged by the operator at PR review (#1246), discharging D1.
- **Recorded:** 2026-08-10T11:10:36+00:00

The verdict ceiling at acceptance time was **rework exists** — D2 and D3 are
`externally-verifiable-later`, and their named re-run conditions were run before
acceptance: `git diff --stat main -- specfuse/loop/triage.py` returned empty, and
`git cat-file -e <parent>:<test path>` failed for all four red-test-first files,
confirming the red half. That evidence is recorded here; the entries below are
still carried forward **verbatim and open**, because accepting a hedge means
