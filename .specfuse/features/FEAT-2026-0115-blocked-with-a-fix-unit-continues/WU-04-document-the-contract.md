...
`blocked_next:`, so the block costs no wait. Mirrors regenerate through
`scripts/sync-scaffold.sh`; `tests/test_scaffold_data_in_sync.py` is the check.
Red-test exempt: documentation; the sync test is the oracle.

**Acceptance criteria.**

1. `grep -c "blocked_next" .specfuse/rules/result-contract.md docs/methodology.md .specfuse/skills/authoring-work-units/SKILL.md`
