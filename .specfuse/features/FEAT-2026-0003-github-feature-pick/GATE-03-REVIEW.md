...
- **T07 = live end-to-end smoke** (against real
  `RestoManagerApp/Backend#287`; the only network-bound step).
...
publishRoster to validated spec") asks for — that work lives in
`RestoManagerApp/Backend`'s tree, not this repo's. T07's deliverable is
...
when an orchestrator dispatches #287 to `RestoManagerApp/Backend`'s
...
`RestoManagerApp/Backend#287` — a real issue in a real production
...
- **Confirm first**: before arming, check the orchestrator's
  addendum (`/Users/christian/RestoManagerApp/orchestrator/docs/`)
...
- Gate 3 (report back + smoke) — `GitHubBackend` label
  ...
  `RestoManagerApp/Backend#287`. **This gate.**
...
### Q1. Is `RestoManagerApp/Backend#287` still the smoke target? (affects T07)
...
```
gh issue view RestoManagerApp/Backend 287 --json
