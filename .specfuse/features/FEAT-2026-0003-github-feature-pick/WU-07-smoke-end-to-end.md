...
`RestoManagerApp/Backend#287` (initiative `INIT-2026-0001/F06`,
...
*"INIT-2026-0001/F06 — 'Conform publishRoster to validated spec'
— RestoManagerApp/Backend issue #287, label specfuse:feature +
...
fix work happens in `RestoManagerApp/Backend`'s loop, not here).
...
edit `RestoManagerApp/Backend`'s tree. Adopt writes inside this
...
1. A new file `SMOKE-INIT-2026-0001-F06.md` exists in the feature
   ...
   - `## Issue state at smoke time` — output of `gh issue view
     RestoManagerApp/Backend 287 --json
   ...
   - `## Discovery step` — `python3 .specfuse/scripts/gh_features.py
     RestoManagerApp/Backend` output captured, with the row for
     #287 highlighted.
...
**Do not touch.**
- `RestoManagerApp/Backend`'s repo or any file under it. The
