...
|---|---|---|
| Section-heading format the orchestrator's issue-body template emits | `RestoManagerApp/orchestrator/` issue-body template / docs / a recent orchestrator-authored `specfuse:feature` issue | UNCHECKED — gate-4 plan-next must verify |
...
| Whether the `state:*` label namespace settled at gate 3 needs any gate-4 follow-up | Already verified at gate 3 (the `state:*` scheme is canonical in `RestoManagerApp/orchestrator/docs/naming-convention.md §5.1`) | OK — no action required |
