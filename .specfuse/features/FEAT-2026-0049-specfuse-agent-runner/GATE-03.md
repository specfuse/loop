...
# Gate 3 — the agent advances features
...
Substantive work units are drafted by gate 2's `plan-next` (`G2-PLAN`). The
terminal `close` WU is scaffolded now as a `draft` placeholder so the linter
reads gate 1 as non-terminal; `G2-PLAN` inserts gate 3's substantive units above
it and sets its real `depends_on`.
...
site-packages rather than the working tree). A gate-3 work unit that imports
...
- **Subprocess invariant.** Confirm no drafted WU imports `loop.run`. This is the
  ...
  when the WUs actually exist, rather than trusting the prose above.
