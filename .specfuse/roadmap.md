...
**Status: active** — terminal close ran with verdict `met_locally`. Two follow-ups stand, classified under the contract this feature ships: the §3 consumer-visible contract-change list needs the operator's acknowledgment (`acceptance-discharged`), and whether the ceiling headline actually helps a human decide faster is an observation no test can make (`externally-verifiable-later`, discharged by the first real `/accept-hedged-close` run — which is this feature's own hedge). Enumeration and both entries are in the feature's `RETROSPECTIVE.md`.
