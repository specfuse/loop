...
**Status: active.** Gate 1's three implementation work units are `done` at $4.57 against $9.00 planned, all three passing first attempt; the terminal close ran with verdict `partially_met`, so the row stays `active` pending the operator's decision at the gate-1 review checkpoint.

**Converge was chosen**, not admit-two-paths. `detect_rearm_dispatch` no longer reads `cost_usd`'s value: a new `folded_through_re_arm` frontmatter integer is compared against `re_arm_count`, and `fold_cumulative_on_rearm` stamps it in the same write set as the four accumulators, so a re-arm whose prior cycle cost $0.00 still folds and a second call for one re-arm is a proven no-op. `cumulative_*` is now unconditionally the lifetime accumulator and `re_arm_history[].prior_*` a pure audit record; `WU.template.md` — both shipped copies — and `cost.py`'s docstring say so. `wu_lifetime_cost_usd` keeps its behaviour and its events-first precedence; only its fallback's documentation changed. The two fold-never-ran units named above were **migrated, not annotated** — the migration ships as `specfuse/loop/rearm_migration.py` so a downstream project can run it against its own records rather than hand-editing them.

**What the hedge is for.** The migration's fold-never-ran branch folds `re_arm_history[].prior_cost_usd` into `cumulative_cost_usd` without resetting `cost_usd`. That is right for a unit re-armed and re-dispatched, and it double-counts a unit re-armed and never re-dispatched — which is WU-04 (FEAT-2026-0020), `completed_out_of_loop: true`, now reading $0.326 against an events total of $0.163. Masked from every current consumer by `wu_lifetime_cost_usd`'s events-first precedence, wrong in the file a human reads, and out of the close's own **Do not touch** boundary to fix. The one-branch fix and the reconciliation that upgrades the verdict to `met` are recorded as FU-1 in the feature's `RETROSPECTIVE.md`. Separately, and pre-existing rather than introduced here: WU-01 (FEAT-2026-0060)'s `cumulative_cost_usd` under-counts its lifetime by $9.23 — the old value guard's damage, frozen in a `done` record that this feature deliberately scoped out of back-filling.
