...
4. **Report back:** state lives in GitHub issue labels + the feature's correlation thread
   ...
   **Live smoke result (T07, 2026-06-06, out-of-loop by human operator):**
   ...
   | Mechanism | Result |
   |-----------|--------|
   | Discovery (`gh_features.py example-org/example-app`) | **PASS** — 13 candidates; `#287` parsed correctly |
   | Adopt (`adopt_feature.py example-org/example-app 287`) | **PASS** — folder + encoding + body embed worked |
