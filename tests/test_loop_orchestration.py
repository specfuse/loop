...
class TestMainArgparse(unittest.TestCase):
    ...
    def test_feature_flag_forwarded_to_run(self):
        ...
        mock_run.assert_called_once_with("my-feature", False, force_full_close=None, prepare=False, prepare_only=False, no_baseline_probe=False)
        ...
        with patch.object(loop, "auto_sync"):
            with patch.object(loop, "run", return_value=0) as mock_run:
                with patch.object(sys, "argv", ["loop", "--dry-run"]):
                    loop.main()
        mock_run.assert_called_once_with(None, True, force_full_close=None, prepare=False, prepare_only=False, no_baseline_probe=False)
